...
void plotAllSlopes(string target="LD2", string spec="HMS")
{
  ...
  */
  TGraph *gr1=plotCarlos(2,7,target.c_str(),specl.c_str());//traacking yield
  TGraph *gr1e=plotCarlos(2,8,target.c_str(),specl.c_str());//traacking yield
  TGraph *gr2=plotCarlos(2,5,target.c_str(),specl.c_str());//non-tracking yield
  TGraph *gr2e=plotCarlos(2,6,target.c_str(),specl.c_str());//non-tracking yield
  TGraph *gr3=plotCarlos(2,3,target.c_str(),specl.c_str());//scaler 
  TGraph *gr3e=plotCarlos(2,4,target.c_str(),specl.c_str());//scaler 
 ...
 TPaveStats *st3=formatit(g_scaler_lh2_shms, 31, 1);
  ...
  g_track_lh2_shms->GetYaxis()->SetRangeUser(.9,1.1);
  g_track_lh2_shms->GetXaxis()->SetRangeUser(3,40);
  ...
  g_track_lh2_shms->GetXaxis()->SetTitle("BCM4A Current (uA)");

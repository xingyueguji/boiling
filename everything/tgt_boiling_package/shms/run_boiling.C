		 ...
		 )
{
  ...
  filename=Form("/volatile/hallc/spring17/wmhenry/f2/boiling/tof100/pass4/shms_replay_production_%d_-1.root", runNum) ;  
  //  filename=Form("/cache/hallc/E12-10-002/abishek/realpass-3d-shms-data/shms_replay_production_%d_-1.root", runNum) ;  
  //  filename=Form("/cache/hallc/E12-10-002/cmorean/pass4-shms-data/shms_replay_production_%d_-1.root", runNum) ;  
  ...
  tdata->SetBranchAddress("P.cal.etracknorm",&hCAL_etottracknorm);    
  ...
  //for (int i = 0; i < 5; i++) {
	...
	if(c_noedtm) {
	  ...
	  */
	  ...
	  if(c_hdelta&&c_etottrknorm&&c_cerNpesum&&c_eprtrknorm&&c_inside&&c_goodscin) //here it is
	    {
    ...
    */
    ...
    //-----SCALER YIELD-----
    //BCM4A
    scl_Yield_bcm4a = (total_ptrig3_scaler_bcm4a_cut-total_hedtm_scaler_bcm4a_cut) / (total_charge_bcm4a_cut);
    scl_err_bcm4a = sqrt(total_ptrig3_scaler_bcm4a_cut) / total_charge_bcm4a_cut;
    //BCM4B
